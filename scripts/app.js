...
function makeDraft() {
  ...
  return {
    ...
    cover: { src:imagePath(coverImage), alt:titleCase(coverImage.name), caption:"" },
    images: articleImages.map((item,index)=>({src:imagePath(item.file),alt:item.description||titleCase(item.file.name),caption:item.description||"",order:index+1}))
  };
}
...
form.addEventListener("submit",event=>{
  ...
  try {
    ...
    output.value="Gemini draft prepared.\n\nThis Build step only prepares the draft; it does NOT publish to Jaziel.\n\nThe GitHub Actions AI Writer will use GEMINI_API_KEY to generate the final Schema v1 article.";
    ...
    empty.hidden=true; result.hidden=false; renderPlan(draft);
  } catch(error) { alert(error.message); }
});

document.getElementById("copyBtn").addEventListener("click",async()=>{await navigator.clipboard.writeText(output.value);const b=document.getElementById("copyBtn");b.textContent="Copied ✓";setTimeout(()=>b.textContent="Copy HTML",1400);});
...
document.getElementById("downloadBtn").addEventListener("click",()=>{const url=URL.createObjectURL(new Blob([output.value],{type:"text/plain"}));const a=document.createElement("a");a.href=url;a.download=`${slug(title.value)||"jaziel-draft"}.txt`;a.click();URL.revokeObjectURL(url);});
